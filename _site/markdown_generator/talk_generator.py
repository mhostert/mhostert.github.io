...
for row, item in talks.iterrows():
    if item.venue != "":
        print(item.venue)
        # Extract date parts - assuming format is MM/DD/YYYY or MM/YYYY
        date_str = str(item.date).strip()
        date_parts = date_str.split("/")
        if len(date_parts) >= 2:
            month = date_parts[0].strip().zfill(2)  # Ensure 2-digit month
            # Check if year is already 4 digits or 2 digits
            year_part = date_parts[-1].strip()  # Get last part (year)
            if len(year_part) == 2:
                year = f"20{year_part}"
            else:
                year = year_part
